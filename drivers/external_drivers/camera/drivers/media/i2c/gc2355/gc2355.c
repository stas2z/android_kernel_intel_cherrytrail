...
static int gc2355_read_reg(struct i2c_client *client,
...
static int gc2355_i2c_write(struct i2c_client *client, u16 len, u8 *data)
{
	struct i2c_msg msg;
	const int num_msg = 1;
	int ret;
...
static int gc2355_write_reg(struct i2c_client *client, u16 data_length,
							u16 reg, u16 val)
{
	int ret;
...
//	MYDBG("%s data_length:0x%x reg:0x%x val:0x%x\n",__func__,data_length,reg,val);
	...
	*wreg = cpu_to_be16(reg);
	...
	ret = gc2355_i2c_write(client, len, data);
	if (ret)
		dev_err(&client->dev,
...
static int gc2355_write_reg_array(struct i2c_client *client,
				  const S_UNI_REG *reglist)
{
	...
	for (; next->type != UNICAM_TOK_TERM; next++) {
		...
		case UNICAM_16BIT:
		case UNICAM_32BIT:
			err=gc2355_write_reg(client, next->type,next->reg,next->val);
			break;
		default:
			break;
		}
...
/*
	tbd:
*/
static int gc2355_get_intg_factor(struct i2c_client *client,
...
static int gc2355_getLZeroBits(int val){
	int i =0;
...
static int gc2355_read_i2c_with_mask(struct i2c_client *client, u16 start_reg, u32 mask,int * value, char * name){
	int ret = 0;
	int val = 0;
	u16 regval = 0;
	int im = 0;
	...
	for(i=0; i<4; i++){
		...
		ret = gc2355_read_reg(client, UNICAM_8BIT,(u16)start_reg+iregidx, &regval);
...
static int gc2355_write_i2c_with_mask(struct i2c_client *client,u16 start_reg, u32 mask,int value, char * name){
	int ret = 0;
	u16 regval = 0;
	int im = 0;
	int i = 0;
	...
	for(i=0; i<4; i++){
		...
		if(im != (0xFF<<(j*8))){//optimize
			ret = gc2355_read_reg(client, UNICAM_8BIT,(u16)start_reg+iregidx, &regval_get);
			MYDBG("reg:0x%x regval:0x%x\n",start_reg+iregidx,regval);
			regval = regval|(regval_get&(~(im>>(j*8))));
		}

		ret = gc2355_write_reg(client, UNICAM_8BIT,(u16)start_reg+iregidx, regval);
		MYDBG("reg:0x%x regval:0x%x \n",start_reg+iregidx,regval);
				 ...
				 int gain, int digitgain)
{
	...
	ret = gc2355_write_reg_array(client, dev->uni->group_hold_start);
	if (ret)
		goto exposure_err;

	/* set lines per frame */
#if 0
	ret = gc2355_write_i2c_with_mask(client,
		dev->uni->exposevts,
	...
	ret = gc2355_write_reg(client, UNICAM_8BIT,
...
static long __d_gc2355_set_exposure(struct v4l2_subdev *sd, int coarse_itg,
				 int gain, int digitgain)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sensor_data *dev = to_sensor_data(sd);
	...
	ret = gc2355_write_i2c_with_mask(client,dev->uni->exposevts,dev->uni->exposevts_mask,vts,"set vts");
	...
	ret = gc2355_write_i2c_with_mask(client,dev->uni->exposecoarse,dev->uni->exposecoarse_mask,coarse_itg,"set exp");
	...
	/* set digital gain */
	ret = gc2355_write_i2c_with_mask(client,dev->uni->exposedigitgainR,dev->uni->exposedigitgain_mask,digitgain,"set dgain R");
	ret += gc2355_write_i2c_with_mask(client,dev->uni->exposedigitgainGR,dev->uni->exposedigitgain_mask,digitgain,"set dgain GR");
	ret += gc2355_write_i2c_with_mask(client,dev->uni->exposedigitgainGB,dev->uni->exposedigitgain_mask,digitgain,"set dgain GB");
	ret += gc2355_write_i2c_with_mask(client,dev->uni->exposedigitgainB,dev->uni->exposedigitgain_mask,digitgain,"set dgain B");
	...
	ret = gc2355_write_reg_array(client, dev->uni->group_hold_end);
	if (ret) {
		dev_err(&client->dev, "unicam write register err.\n");
		mutex_unlock(&dev->input_lock);
		return ret;
...
#if 0
static int gc2355_set_exposure(struct v4l2_subdev *sd, int exposure,
	int gain, int digitgain)
{
	struct sensor_data *dev = to_sensor_data(sd);
	int ret;
...
//	mutex_lock(&dev->input_lock);
	ret = __d_gc2355_set_exposure(sd, exposure, gain, digitgain);
//	mutex_unlock(&dev->input_lock);

	return ret;
}
#endif
static long gc2355_s_exposure(struct v4l2_subdev *sd,
			       struct atomisp_exposure *exposure)
{
	...
	default:
		return __d_gc2355_set_exposure(sd, exp, gain, digitgain);
	}
}
...
static long gc2355_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case ATOMISP_IOC_S_EXPOSURE:
		return gc2355_s_exposure(sd, arg);
	default:
		return -EINVAL;
	}
	return 0;
...
static int gc2355_init(struct v4l2_subdev *sd)
{
	struct sensor_data *dev = to_sensor_data(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
	...
	ret = gc2355_write_reg_array(client, dev->uni->global_setting);
	if (ret) {
		dev_err(&client->dev, "unicam write register err.\n");
		return ret;
	}
...
static int gc2355_s_power(struct v4l2_subdev *sd, int on)
{
	int ret;
...
static int startup(struct v4l2_subdev *sd)
{
	...
	ret = gc2355_write_reg_array(client, dev->cur_res[dev->fmt_idx].regs);
	if (ret) {
		dev_err(&client->dev, "unicam write register err.\n");
		return ret;
	}
			     ...
			     struct v4l2_mbus_framefmt *fmt)
{
	struct sensor_data *dev = to_sensor_data(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct camera_mipi_info *gc2355_info = NULL;
	int ret = 0;

	MYDBGIN();
	gc2355_info = v4l2_get_subdev_hostdata(sd);
	if (gc2355_info == NULL)
		return -EINVAL;
	...
	ret = gc2355_try_mbus_fmt(sd, fmt);
	if (ret == -1) {
	...
	ret = gc2355_get_intg_factor(client, gc2355_info,
...
static int gc2355_g_mbus_fmt(struct v4l2_subdev *sd,
			     struct v4l2_mbus_framefmt *fmt)
{
	struct sensor_data *dev = to_sensor_data(sd);
...
static int gc2355_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	ret = gc2355_write_reg_array(client, enable ? dev->uni->stream_on: dev->uni->stream_off);
...
static int gc2355_enum_mbus_fmt(struct v4l2_subdev *sd,
				unsigned int index,
...
static int det_id(struct i2c_client *client)
{
	...
	if (dev->uni->flag & UNI_DEV_FLAG_1B_ACCESS) {
		if (len == 2) {
			MYDBG("2 1B access\n");
			ret = gc2355_read_reg(client, UNICAM_8BIT,
					dev->uni->idreg, &vp);
			val |= (vp & 0xff) << 8;
			ret = gc2355_read_reg(client, UNICAM_8BIT,
					dev->uni->idreg + 1, &vp);
...
static struct gc2355_vcm * find_vcm_mapping(int id){
	int find = 0;
	const struct gc2355_vcm_mapping *mapping = gc2355_supported_vcms;
	while (mapping != NULL) {
		if (mapping->id == id){
			/* compare name */
			find = 1;
...
static int gc2355_s_config(struct v4l2_subdev *sd,
			   int irq, void *pdata)
{
	struct sensor_data *dev = to_sensor_data(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
//	}
	...
	ret = det_id(client);
	if (ret) {
		dev_err(&client->dev, "gc2355_detect err s_config.\n");
...
static int gc2355_g_parm(struct v4l2_subdev *sd,
...
static int gc2355_s_parm(struct v4l2_subdev *sd,
			struct v4l2_streamparm *param)
{
	struct sensor_data *dev = to_sensor_data(sd);
...
static int gc2355_enum_frame_size(struct v4l2_subdev *sd,
				struct v4l2_subdev_fh *fh,
...
static int gc2355_get_pad_format(struct v4l2_subdev *sd,
				struct v4l2_subdev_fh *fh,
...
static int gc2355_set_pad_format(struct v4l2_subdev *sd,
				struct v4l2_subdev_fh *fh,
				struct v4l2_subdev_format *fmt)
{
	struct sensor_data *dev = to_sensor_data(sd);
...
static int gc2355_g_skip_frames(struct v4l2_subdev *sd, u32 *frames)
{
	struct sensor_data *dev = to_sensor_data(sd);
...
static int gc2355_q_exposure(struct v4l2_subdev *sd, s32 *value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sensor_data *dev = to_sensor_data(sd);
	int val=0;
	...
	switch (dev->uni->idval) {
	case 0x2355:
		ret = gc2355_write_reg(client, UNICAM_8BIT, 0xFE, 0x00);
		if (ret) {
			dev_err(&client->dev, "unicam gc2355_q_exposure err.\n");
			return ret;
		}
		ret = gc2355_read_i2c_with_mask(client, dev->uni->exposecoarse, dev->uni->exposecoarse_mask, &val, "q_exp");
		if (ret) {
			dev_err(&client->dev, "unicam gc2355_q_exposure err.\n");
			return ret;
		}
...
int gc2355_t_focus_abs(struct v4l2_subdev *sd, s32 value)
{
	struct sensor_data *dev = to_sensor_data(sd);
	int ret = 0;
...
int gc2355_t_focus_rel(struct v4l2_subdev *sd, s32 value)
{
	struct sensor_data *dev = to_sensor_data(sd);
	int ret = 0;
...
int gc2355_q_focus_status(struct v4l2_subdev *sd, s32 *value)
{
	struct sensor_data *dev = to_sensor_data(sd);
	int ret = 0;
...
int gc2355_q_focus_abs(struct v4l2_subdev *sd, s32 *value)
{
	struct sensor_data *dev = to_sensor_data(sd);
	int ret = 0;
...
static int gc2355_g_focal(struct v4l2_subdev *sd, s32 *val)
{
	*val = (UNICAM_FOCAL_LENGTH_NUM << 16) | UNICAM_FOCAL_LENGTH_DEM;
	return 0;
}

static int gc2355_g_fnumber(struct v4l2_subdev *sd, s32 *val)
{
...
static int gc2355_g_fnumber_range(struct v4l2_subdev *sd, s32 *val)
{
...
static int gc2355_g_bin_factor_x(struct v4l2_subdev *sd, s32 *val)
{
	struct sensor_data *dev = to_sensor_data(sd);
...
static int gc2355_g_bin_factor_y(struct v4l2_subdev *sd, s32 *val)
{
	struct sensor_data *dev = to_sensor_data(sd);
...
struct gc2355_control gc2355_controls[] = {
	{
		.qc = {
	...
	},
	{
		...
		.tweak = gc2355_t_focus_abs,
		.query = gc2355_q_focus_abs,
	...
	},
	{
		...
		.query = gc2355_g_bin_factor_y,
	},
};

#define N_CONTROLS (ARRAY_SIZE(gc2355_controls))

static struct gc2355_control *gc2355_find_control(u32 id)
{
	int i;

	for (i = 0; i < N_CONTROLS; i++)
		if (gc2355_controls[i].qc.id == id)
			return &gc2355_controls[i];
	return NULL;
}

static int gc2355_queryctrl(struct v4l2_subdev *sd, struct v4l2_queryctrl *qc)
{
	struct gc2355_control *ctrl = gc2355_find_control(qc->id);
	struct sensor_data *dev = to_sensor_data(sd);
...
static int gc2355_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	s_ctrl = gc2355_find_control(ctrl->id);
...
{
	struct gc2355_control *octrl = gc2355_find_control(ctrl->id);
	struct sensor_data *dev = to_sensor_data(sd);
	int ret;
...
static const struct v4l2_subdev_sensor_ops gc2355_sensor_ops = {
	.g_skip_frames	= gc2355_g_skip_frames,
};
...
static const struct v4l2_subdev_pad_ops gc2355_pad_ops = {
	...
	.enum_frame_size = gc2355_enum_frame_size,
	.get_fmt = gc2355_get_pad_format,
	.set_fmt = gc2355_set_pad_format,
};
...
static int gc2355_remove(struct i2c_client *client)
{
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct sensor_data *dev = to_sensor_data(sd);
	dev_dbg(&client->dev, "gc2355_remove...\n");
	MYDBGNP("gc2355_remove...\n");
...
//	const struct firmware *fw;
	struct match_data *gc2355_data = NULL;
//	S_UNI_DEVICE *fwdev=NULL;
	void *pdata = client->dev.platform_data;
	static int gc2355_u_data = 0;

	MYDBG("gc2355_probe name: %s, gc2355_u_data = %d\n",
			client->name, gc2355_u_data);
	...
	if (client->name &&
		...
		gc2355_data = gc2355_m_data_front;
	else
		gc2355_data = gc2355_m_data;

	/* ret = request_firmware(&fw, UNI_FW_PATH, &client->dev); */
	if (1) {
		MYDBG("not find firmware PPR\n");
		for (i = 0; gc2355_data[i].uni != NULL; i++) {
			dev->uni = gc2355_data[i].uni;
			MYDBG("gc2355_probe name: gc2355_data[%d].name %s\n",
					i,
					gc2355_data[i].name);
			mutex_init(&dev->input_lock);
			dev->fmt_idx = 0;
			v4l2_i2c_subdev_init(&(dev->sd), client, &gc2355_ops);
		...
		ret = gc2355_s_config(&dev->sd, client->irq,
			       pdata);
		if (ret) {
			if(gc2355_data[i+1].uni==NULL) {
				goto out_free;}

			v4l2_device_unregister_subdev(&dev->sd);
		}else{
//			ret = gc2355_atomisp_register_i2c_module(&dev->sd, pdata, RAW_CAMERA);
...
//ppr no module
MODULE_DEVICE_TABLE(i2c, gc2355_cam);

static struct acpi_device_id gc2355_acpi_match[] = {
#if 0
	{"INT33F3"},
	{"HIMX8131"},
	{"OVTI8865"},
...
MODULE_DEVICE_TABLE(acpi, gc2355_acpi_match);

static struct i2c_driver gc2355_ppr_cam = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "gc2355",
		.acpi_match_table = ACPI_PTR(gc2355_acpi_match),
	},
	.probe = gc2355_probe,
...
{
	MYDBG("%s\n",__func__);
	return i2c_add_driver(&gc2355_ppr_cam);
}

static __exit void exit_gc2355(void)
{
	i2c_del_driver(&gc2355_ppr_cam);
}

module_init(init_gc2355);
