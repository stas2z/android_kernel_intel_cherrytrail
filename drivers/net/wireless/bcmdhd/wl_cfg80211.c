...
get_station_err:
		if (err && (err != -ERESTARTSYS)) {
			...
			cfg80211_disconnected(dev, 0, NULL, 0, 0, GFP_KERNEL);
			wl_link_down(cfg);
		}
	}
	else {
			...
			if (wl_get_drv_status(cfg, CONNECTED, ndev)) {
				...
				if (!wl_get_drv_status(cfg, DISCONNECTING, ndev)) {
					...
					*/
					...
					cfg80211_disconnected(ndev, reason, NULL, 0, 0, GFP_KERNEL);
					wl_link_down(cfg);
					wl_init_prof(cfg, ndev);
				} else {
...
int wl_cfg80211_hang(struct net_device *dev, u16 reason)
{
	...
	wl_add_remove_pm_enable_work(cfg, FALSE, WL_HANDLER_DEL);
	cfg80211_disconnected(dev, reason, NULL, 0, 0, GFP_KERNEL);
	if (cfg != NULL) {
		wl_link_down(cfg);
	}
	return 0;
